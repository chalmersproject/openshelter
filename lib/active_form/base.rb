...
module ActiveForm
  ...
  module Base
    ...
    included do
      ...
      #
      class << self
        def alias_param(alias_key, attribute_key)
        ...
        def attribute(name, type = ActiveModel::Type::Value.new, **options)
          ...
          if attribute_type.is_a?(ActiveModel::Type::Boolean)
            alias_boolean(name)
          end
    ...
    def process_params_hash(params_hash)
    ...
    # the parameter here
    def dealias_params(params_hash)
      self.class.param_aliases.each do |alias_key, attribute_key|
        next if (merged_values = params_hash[alias_key]).blank?
    ...
    # the form
    def merge_date_and_time_params(params_hash)
