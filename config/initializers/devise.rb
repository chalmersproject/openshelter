...
Devise.setup do |config|
  ...
  # config.secret_key = '19c8104d461e629aa1bc90e56dd1c567721c93ab4a451f303a3428afab75a4c8292a25fe06a22e07e50a5676b45d187246d0720f0bee4bd7bcfae937151531f7'
  ...
  # config.mailer_sender = "Chalmers Robot <robot@chalmersproject.com>"
  ...
  # filter. You can also supply a hash where the value is a boolean determining whether
  ...
  # used on authentication. The same considerations mentioned for authentication_
  # keys also apply to request_keys.
  # config.request_keys = []
  ...
  # will likely want to enable this with :database unless you are using a custom
  # strategy. The supported strategies are:
  ...
  # When false, Devise will not attempt to reload routes on eager load. This can
  # reduce the time taken to boot the app, but if your application requires the
  # Devise mappings to be loaded during boot time the application won't boot
  # properly.
  ...
  # hashed. The number of stretches used for generating the hashed password are
  # stored with the hashed password. This allows you to change the stretches
  ...
  # config.pepper = '141da3a2fb259c8b2eb9c80d63ce87cc403238922caf3fadbfbec9739e125d249775c553414215398d567126b8b60a4c0ecd2430868a64f8f6c7d8a9d254d028'
  ...
  # Configure sign-ins with Google.
  scoped do
    ...
    if [client_id, client_secret].all?(&:present?)
      config.omniauth(
        ...
        client_secret,
        name: :google,
        scope: %w[userinfo.email userinfo.profile].join(", "),
      )
    end
  end

  # Configure sign-ins with Facebook.
  scoped do
    app_id = ENV["FACEBOOK_APP_ID"]
    app_secret = ENV["FACEBOOK_APP_SECRET"]
    if [app_id, app_secret].all?(&:present?)
      config.omniauth(
        :facebook,
        app_id,
        app_secret,
        scope: %w[email public_profile].join(","),
        info_fields: %w[email name first_name last_name picture].join(", "),
      )
    end
  end
